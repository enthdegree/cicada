...
def run_payload_verification(payload_cls, args: argparse.Namespace, frames_csv: Path, output_md: Path):
	...
	annotated_md = ""
	if args.input_md:
		transcript_text = verification.load_markdown_transcript(args.input_md)
		l_tokens = speech.regularize_transcript(transcript_text)
		l_match_idx = []
		for pl in l_payloads:
			kwargs = {}
			if bls_pubkey_bytes is not None:
				kwargs["bls_pubkey_bytes"] = bls_pubkey_bytes
			l_match_idx.append(pl.match_chunk(transcript_text, **kwargs))
		chunk_md = verification.annotate_chunk(transcript_text, l_tokens, l_payloads, l_match_idx, l_payload_start_sam, wav_fs_Hz=44100.0)
	...
	annotated_md += verification.write_appendix_md(l_payloads, l_payload_start_sam, 44100.0)
...
def main(argv: list[str] | None = None):
	...
	if args.output_md is None:
		default_transcript = Path(f"{Path(args.input_wav).stem}_transcript.md")
