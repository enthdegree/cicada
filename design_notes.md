...
The audio signature player is a browser webapp. 
...
We get a recording of some speech that had signatures in it.
The verifier knows and trusts the signer's public key.

From this recording the listener recovers a transcript of the speech, and all the signatures.
The listener checks segments of the transcript to see if they match the signatures.
...
Set some symbols to parameterize FSK: 

- $M$, bits per FSK symbol
...
The bottom two are beyond the limits of the available frequency resolution we guessed above, so we pick $M=1$.
...
- We've decided on pulse processing so it makes more sense to approximate channel capacity after integrating pulses.
  ...
  - At 196 real samples (98 complex) $x=0.539$ so we take 2.69 dB loss due to frequency shift. The upper bound in this condition is $794 \ \mathrm{bits}/\mathrm{sec}$.

The parameters we chose above are 8-10 dB beneath this estimate.

## Prior work 

### Physical-layer watermarking
This is not a unique idea. As a starting reference, a team at Cornell is working on a similar appliance for images[1].

### Acoustic communication
It would have been nice to use Quiet[2] instead of rolling our own FSK mod + LDPC coding. 
...
Aerial acoustic communication surveys[3] suggest state-of-the-art inaudible long-distance waveforms achieve ~20 bits/sec using very prominent features (frequency-shift keying, chirp spread spectrum) and sophisticated waveform designs.
We aim for 10x this rate but may have better channel conditions: communication beyond ~10 m is an unlikely use case. 
### References 

[1]: https://bowers.cornell.edu/news-stories/hiding-secret-codes-light-protects-against-fake-videos
[2]: Quiet modem here: https://quiet.github.io/docs/org.quietmodem.Quiet/ ; Quiet reliable profiles here: https://github.com/quiet/quiet-js/blob/master/quiet-profiles.json
[3]: Lee et. al. "Chirp signal-based aerial acoustic communication for smart devices" here: https://ieeexplore.ieee.org/abstract/document/7218629
