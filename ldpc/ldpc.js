// ldpc/ldpc_encoder_simple.js
// QC-IRA LDPC encoder (n=1024, k=512). No deps. Works in browser (and Node).
// Public API:
//   const enc = new Encoder1024R12();
//   const cw = enc.encode(u);  // u: Uint8Array length 512, returns Uint8Array length 1024
...
export class Encoder1024R12 {
  constructor(Z = 32) {
    ...
    this.Hu = this._buildHu(); // row -> array of info columns [0..K-1]
    // scratch buffers
  ...
  encode(u) {
    ...
    for (let r = 0; r < this.M; r++) {
      let acc = 0;
      const cols = this.Hu[r];
      for (let i = 0; i < cols.length; i++) acc ^= (u[cols[i]] & 1);
      this._s[r] = acc;
    }

    // p[0] = rot(s[0]); p[i] = rot(s[i] XOR p[i-1])
    this._p.fill(0);
    ...
    // assemble [u | p]
    this._cw.set(u, 0);
    this._cw.set(this._p, this.K);
    return this._cw.slice();
  }

  _buildHu() {
    const Z = this.Z, BR = this.BR, info_bc = this.BC - this.BR; // 16
    ...
    for (let i=0;i<BR;i++) {
      for (let t=0;t<6;t++) {
        ...
        for (let z=0; z<Z; z++) {
          const r = i*Z + z;
          const c = j*Z + ((z + shift) % Z); // info column
          rows[r].push(c);
        }
      }
    }
    for (let r=0; r<rows.length; r++) {
      rows[r].sort((a,b)=>a-b);
      const out = [];
      for (let i=0, last=-1; i<rows[r].length; i++) {
        const v = rows[r][i]; if (v !== last) out.push(v), last = v;
      }
      rows[r] = out;
    }
    return rows;
  ...
  _rotXorInto(src, sOff, dst, dOff, r) {
    ...
    if (s === 0) {
      for (let i=0;i<Z;i++) dst[dOff+i] ^= src[sOff+i];
    } else {
      for (let i=0;i<Z;i++) {
        const j = (i - s + Z) % Z; // right-rotate
        dst[dOff+i] ^= src[sOff+j];
      }
    }
  }

  _rotateInPlace(a, off, len, r) {
    const Z = len, s = ((r%Z)+Z)%Z;
    if (s === 0) return;
    const tmp = a.slice(off+Z-s, off+Z);
    a.copyWithin(off+s, off, off+Z-s);
    a.set(tmp, off);
  }
}
